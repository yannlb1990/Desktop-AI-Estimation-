...
// Compute points along an arc (in DXF world coords) using line-segment approximation.
// All angles are in radians, standard math convention (CCW positive, from +X).
function arcPoints(cx: number, cy: number, r: number, startAngle: number, endAngle: number, ccw: boolean, steps = 32): Vec2[] {
...
// Convert LWPOLYLINE bulge value to arc geometry (all in DXF world coords).
function bulgeArc(x1: number, y1: number, x2: number, y2: number, bulge: number) {
  ...
  // Left-perpendicular of direction p1→p2
...
function expandBBox(b: BBox, x: number, y: number) {
...
function entityBBox(e: any): BBox | null {
  ...
  switch (e.type) {
    ...
    case 'ARC':
      // Include center ± radius for a conservative bound
...
function renderEntities(
  ...
  for (const e of entities) {
    ...
    switch (e.type) {
      ...
      case 'LWPOLYLINE': {
        ...
        for (let i = 0; i < verts.length - 1; i++) {
          ...
          if (Math.abs(bulge) > 1e-6) {
            const arc = bulgeArc(v.x, v.y, vn.x, vn.y, bulge);
            if (arc) {
        ...
        if (e.shape) {
          // Closed polyline — segment from last vertex back to first
          const last = verts[verts.length - 1];
          const first = verts[0];
          const bulge = last.bulge ?? 0;
          if (Math.abs(bulge) > 1e-6) {
            const arc = bulgeArc(last.x, last.y, first.x, first.y, bulge);
            if (arc) {
              for (const pt of arcPoints(arc.cx, arc.cy, arc.r, arc.startAngle, arc.endAngle, arc.ccw)) {
                ctx.lineTo(tx(pt.x), ty(pt.y));
              }
      ...
      case 'ARC': {
        // DXF arc: CCW from startAngle to endAngle (degrees from +X)
        const { center, radius, startAngle, endAngle } = e;
        const sa = startAngle * Math.PI / 180;
        const ea = endAngle * Math.PI / 180;
        const pts = arcPoints(center.x, center.y, radius, sa, ea, true /* CCW in DXF */);
      ...
      case 'ELLIPSE': {
        ...
        for (let i = 0; i <= steps; i++) {
          ...
          // Ellipse point in local coords then rotate by rot
      ...
      case 'TEXT': case 'MTEXT': {
        ...
        // Strip MTEXT control codes: {\...} and \P (paragraph break)
        const clean = (e.text as string).replace(/\{[^}]*\}/g, '').replace(/\\P/gi, ' ');
        ctx.fillText(clean, tx(pos.x), ty(pos.y));
        break;
      }
...
export async function renderDxfToBlob(
  ...
  // Bounding box across all entities
  ...
  if (!isFinite(bbox.minX)) throw new Error('DXF file contains no renderable geometry');

  const dxfW = bbox.maxX - bbox.minX;
  const dxfH = bbox.maxY - bbox.minY;
  if (dxfW < 1e-6 || dxfH < 1e-6) throw new Error('DXF bounding box is too small');
  ...
  // DXF: Y up. Canvas: Y down. Flip Y around bbox.maxY.
